...
class AdaptiveGroupEstimator(nn.Module):
    def __init__(self, function_type=1, tau=0.5):
        """
        tau: fixed threshold for the affinity matrix.
        """
        super().__init__()
    ...
    def forward(self, node_features):
        # node_features: (B, N, d)
        temp = F.normalize(node_features, p=2, dim=2)
        corr_mat = torch.matmul(temp, temp.permute(0, 2, 1))  # (B, N, N) cosine similarity
        # Directly threshold the affinity matrix
        G = (corr_mat >= self.tau).float()
        return G
